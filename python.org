...
* Table of Contents                                                :TOC_2_gh:
 ...
 - [[#execute-subprocesses][Execute subprocesses]]
   - [[#calling-an-external-command-in-python][Calling an external command in Python]]
   - [[#use-pipe-along-with-communicate][Use PIPE along with communicate()]]
...
#+BEGIN_EXAMPLE
  ...
  issubset(...)
  issuperset(...)
  pop(...)                          remove and return an arbitrary elem; raises KeyError if empty
  remove(...)                       remove an elem; raises KeyError if elem is not a member
  symmetric_difference(...)
  symmetric_difference_update(...)  all elems that are in exactly one of the sets
  union(...)
  update(...)
#+END_EXAMPLE
